...
vehicle_detector = YOLO('models/yolov8n.pt')
...
csv_file = open('license_plate_detections.csv', mode='w', newline='')
...
while True:
    ...
    for license_plate in license_plate_detections.boxes.data.tolist():
        ...
        if assigned_car != -1:
            ...
            license_plate_image_path = f'license_plates/frame_{frame_count}_vehicle_{car_id}.jpg'
            ...
            processed_image_path = f'processed_license_plates/frame_{frame_count}_vehicle_{car_id}_processed.jpg'
...
csv_file = 'license_plate_detections.csv'
...
out = cv2.VideoWriter('annotated_video.mp4', fourcc, 30.0, (int(cap.get(3)), int(cap.get(4))))
...
with open(csv_file, 'r') as file:
    ...
    for row in csv_reader:
        ...
        bbox_vehicle = list(map(int, bbox_vehicle.strip('[]').split(',')))
        bbox_license_plate = list(map(int, bbox_license_plate.strip('[]').split(',')))
        ...
        if int(frame_number) != frame_count:
            ...
            if current_frame_data:
                ...
                for vehicle_data in current_frame_data:
                    ...
                    cv2.rectangle(frame, (vehicle_bbox[0], vehicle_bbox[1]), (vehicle_bbox[2], vehicle_bbox[3]),
                                  (255, 0, 0), 2)
                    cv2.putText(frame, f'Vehicle {vehicle_id}', (vehicle_bbox[0], vehicle_bbox[1] - 10),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)

                    # Draw bounding box for the license plate
                    cv2.rectangle(frame, (license_plate_bbox[0], license_plate_bbox[1]),
                                  (license_plate_bbox[2], license_plate_bbox[3]), (0, 255, 0), 2)
                    cv2.putText(frame, f'Plate {vehicle_id}', (license_plate_bbox[0], license_plate_bbox[1] - 10),
